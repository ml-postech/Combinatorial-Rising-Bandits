...
class Algo(BaseAlgo):
    ...
    def get_actions(self, ob, bg, a_max=1, random_goal=False, act_randomly=False, graph=False):
        ...
        self.waypoint_subgoal = self.graphplanner.get_waypoint(ob, self.curr_subgoal, bg)[:self.subgoal_dim]
    ...
    def run_eval(self, epoch, use_test_env=False, render=False):
        ...
        for n_test in range(self.args.n_test_rollouts):
                ...
                if n_test==0:
                    ...
                    if self.graphplanner.graph is not None:
                        if (bef_subgoal != self.curr_subgoal).any():
                            graph_image, _ = self.graphplanner.draw_graph(start=ag, subgoal=self.curr_subgoal)
            ...
            if n_test == 0:
                ...
                self.monitor.store_video(images, graph_images, graph_image, edge_graph_image)
    ...
    def eval_get_actions(self, ob, bg, a_max=1, random_goal=False, act_randomly=False, graph=False):
        ...
        # which waypoint to chase
        self.waypoint_subgoal = self.graphplanner.get_waypoint(ob, self.curr_subgoal, bg)
        act = self.low_agent.get_actions(ob, self.waypoint_subgoal)
        self.way_to_subgoal -= 1 
        return act

    def inference(self):
        self.load_all(self.model_path)
        self.graphplanner.graph_construct(1)
        her_success = self.run_eval(1, use_test_env=True, render=self.args.eval_render)
        self.monitor.store(Success_Rate=her_success)
        pass
